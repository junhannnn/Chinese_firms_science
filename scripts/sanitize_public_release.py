...
MANIFEST_PATH = ROOT / "config" / "processed_data_manifest.csv"
...
def check_manifest() -> CheckResult:
    if not MANIFEST_PATH.exists():
        return CheckResult("processed manifest", False, [f"{rel(MANIFEST_PATH)} is missing"])
...
def check_document_references() -> CheckResult:
    ...
    details: list[str] = []
    scan_roots = sorted(path for path in iter_project_files() if path.suffix.lower() == ".md")
    scan_roots.append(ROOT / "scripts")
        ...
        for path in files:
            ...
            for lineno, line in enumerate(path.read_text(encoding="utf-8", errors="ignore").splitlines(), start=1):
                for match in path_like.finditer(line):
                    # Preserve leading dots in relative paths such as ../README.md.
    ...
    return CheckResult("documentation references", True, ["README/scripts references resolve or are documented exclusions"])
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="rebuild config/processed_data_manifest.csv from proc_output/ and stata_output/",
    )
