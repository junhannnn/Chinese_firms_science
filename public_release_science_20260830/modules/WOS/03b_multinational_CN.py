...
# CONFIG
# -------------------------------------------------------------------
INPUT_PATH = "03_WOS_unique_orgs_firms_CN.parquet"
OUTPUT_CSV = "03_WOS_unique_orgs_firms_CN_filtered.csv"
MODEL = "llama3.3:70b"
...
1. The organization's global headquarters is in China.
2. The affiliation (if present) is also located in China.

Output rules:
- Return "1" only if the organization is headquartered in China AND the affiliation
  (if any) is clearly in China.
- Return "0" in all other cases:
  - if the organization is not headquartered in China,
  - if the affiliation is outside China,
...
Examples:
- "Tencent" -> 1                  (HQ in China; no contrary location given)
- "Huawei California" -> 0        (HQ in China; affiliation in U.S.)
- "Alibaba (Hangzhou)" -> 1       (Both HQ and affiliation in China)
- "Siemens Beijing" -> 0          (Siemens HQ not in China)
- "ByteDance Singapore" -> 0      (HQ in China; affiliation outside China)
""".strip()
