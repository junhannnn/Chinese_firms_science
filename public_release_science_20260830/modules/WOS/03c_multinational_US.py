...
# CONFIG
# -------------------------------------------------------------------
INPUT_PATH = "03_WOS_unique_orgs_firms_US.parquet"
OUTPUT_CSV = "03_WOS_unique_orgs_firms_US_filtered.csv"
MODEL = "llama3.3:70b"
...
optionally followed by a location (affiliation). 

Task:
Determine whether BOTH of the following are true:
1. The organization's global headquarters is in the United States.
2. The affiliation (if present) is also located in the United States.

Output rules:
- Return "1" only if the organization is headquartered in the United States AND the
  affiliation (if any) is clearly in the United States.
- Return "0" in all other cases:
  - if the organization is not headquartered in the U.S.,
  - if the affiliation is outside the U.S.,
  - or if you are uncertain about either.
...
Examples:
- "Google" -> 1                    (HQ in U.S.; no contrary location given)
- "Microsoft (Seattle)" -> 1       (Both HQ and affiliation in U.S.)
- "Apple Beijing" -> 0             (HQ in U.S.; affiliation in China)
- "Samsung New Jersey" -> 0        (Samsung HQ not in U.S.)
- "IBM London" -> 0                (HQ in U.S.; affiliation outside U.S.)
""".strip()
